...
class RemoteDataSource implements IRemoteDataSource {
  ...
  Future<List<Event>> getEvents() async {
    final uri = Uri.parse(
      '$baseUrl/$contractKey/data/events/all',
    ...
    try {
      events =
          wrapperList.map((wrapper) {
            final dataMap =
                (wrapper as Map<String, dynamic>)['data']
                    as Map<String, dynamic>;
            return Event.fromMap(dataMap);
          }).toList();
    } catch (e, st) {
      debugPrint(' Error while parsing events: $e');
  ...
  Future<List<EventTrack>> getEventTracks() async {
    ...
    final request = Uri.parse(
      '$baseUrl/$contractKey/data/event_tracks/all',
    ...
    if (response.statusCode == 200) {
      ...
      final List<Map<String, dynamic>> onlyData =
      ...
      tracks = List<EventTrack>.from(
        onlyData.map((track) => EventTrack.fromMap(track)),
      );
    } else {
  ...
  Future<void> addEventReview(EventReview eventReview) async {
    final request = Uri.parse(
      '$baseUrl/$contractKey/data/store',
    ).resolveUri(Uri(queryParameters: {'format': 'json'}));
  ...
  Future<List<EventReview>> getEventReviews(int eventId) async {
    ...
    final request = Uri.parse(
      '$baseUrl/$contractKey/data/event_reviews/all?format=json&event_id=$eventId',
    ).resolveUri(Uri(queryParameters: {'format': 'json'}));
    final response = await httpClient.get(request);
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body)['data'];
      ...
      reviews = List<EventReview>.from(
        onlyData.map((review) => EventReview.fromMap(review)),
      );
    } else {
      print('Error: ${response.statusCode}');
      print('Body: ${response.body}');
  ...
  Future<void> uploadEvent(Event event) async {
    final request = Uri.parse(
      '$baseUrl/$contractKey/data/events',
    ).resolveUri(Uri(queryParameters: {'format': 'json'}));
    final response = await httpClient.post(
      request,
      headers: {'Content-Type': 'application/json'},
